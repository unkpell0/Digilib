<x-app-layout>
    ...
    <div class="w-full mx-auto bg-white shadow-md p-6 border-l-4 border-white">
        <!-- Search Bar -->
        <form action="/dashboard/search" method="GET" enctype="multipart/form-data">
            <div class="flex justify-end mb-6 p-2 rounded-full">
                <div class="flex items-center max-w-md space-x-2 bg-gray-200 py-2 px-3 rounded-full">
                    <input type="search" name="search" placeholder="Search" class="flex-grow p-2 bg-white-200 focus:outline-none rounded-full focus:ring-0">
        ...
        <div class="grid grid-cols-4 gap-4 mx-auto w-full max-w-screen-lg">
            @if(isset($search))
                <h2 class="col-span-4 text-center text-lg font-semibold">Hasil Pencarian untuk: "{{ $search }}"</h2>
            @endif
