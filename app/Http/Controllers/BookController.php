...
class BookController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $bookCoverPath = $request->file('book_cover')
            ? $request->file('book_cover')->storeAs('storage/app/public/book_covers', Str::random(40) . '.' . $request->file('book_cover')->extension(), 'public')
            : null;

        $pdfFilePath = $request->file('pdf_file')
            ? $request->file('pdf_file')->storeAs('storage/app/public/pdf_files', Str::random(40) . '.' . $request->file('pdf_file')->extension(), 'public')
            : null;
    ...
    {
        // Delete book cover if it exists
        if ($book->book_cover && Storage::disk('public')->exists($book->book_cover)) {
            Storage::disk('public')->delete($book->book_cover);
        }
